...
/// Usage: instantiate Tospb_PingServiceClient, then call methods of this protocol to make API calls.
public protocol Tospb_PingServiceClientProtocol: GRPCClient {
    func sendPing(
        _ request: Tospb_Ping,
        callOptions: CallOptions?
    ) -> UnaryCall<Tospb_Ping, Tospb_Pong>
}

public extension Tospb_PingServiceClientProtocol {
    /// Unary call to SendPing
    ///
    /// - Parameters:
    ///   - request: Request to send to SendPing.
    ///   - callOptions: Call options.
    /// - Returns: A `UnaryCall` with futures for the metadata, status and response.
    func sendPing(
        _ request: Tospb_Ping,
        ...
        return makeUnaryCall(
            path: "/tospb.PingService/SendPing",
            request: request,
            callOptions: callOptions ?? defaultCallOptions
        )
    }
...
public final class Tospb_PingServiceClient: Tospb_PingServiceClientProtocol {
    public let channel: GRPCChannel
    public var defaultCallOptions: CallOptions

    /// Creates a client for the tospb.PingService service.
    ///
    /// - Parameters:
    ///   - channel: `GRPCChannel` to the service host.
    ///   - defaultCallOptions: Options to use for each service call if the user doesn't provide them.
...
/// To build a server, implement a class that conforms to this protocol.
public protocol Tospb_PingServiceProvider: CallHandlerProvider {
    func sendPing(request: Tospb_Ping, context: StatusOnlyCallContext) -> EventLoopFuture<Tospb_Pong>
}

public extension Tospb_PingServiceProvider {
    var serviceName: Substring { return "tospb.PingService" }
    ...
    func handleMethod(_ methodName: Substring, callHandlerContext: CallHandlerContext) -> GRPCCallHandler? {
        ...
        case "SendPing":
            return CallHandlerFactory.makeUnary(callHandlerContext: callHandlerContext) { context in
                { request in
                    self.sendPing(request: request, context: context)
                }
            }
